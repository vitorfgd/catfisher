...
export interface LeaderboardAdapter {
  ...
  submitFishCaught(score: number): LeaderboardState;
}
...
export class BrowserFakeLeaderboardAdapter implements LeaderboardAdapter {
  ...
  private readNumber(field: 'best' | 'last'): number {
    try {
      if (typeof localStorage === 'undefined') return 0;
      const raw = localStorage.getItem(LEADERBOARD_KEY);
      if (!raw) return 0;
      const parsed = JSON.parse(raw) as Partial<Record<'best' | 'last', number>>;
  ...
  private write(best: number, last: number): void {
    try {
      if (typeof localStorage === 'undefined') return;
      localStorage.setItem(LEADERBOARD_KEY, JSON.stringify({ best, last }));
    } catch {
      /* private mode, etc. */
    }
  }
